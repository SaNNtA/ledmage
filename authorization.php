...
<head>
    ...
    <title>Ледмаге | авторизация</title>
...
<?php
session_start();
if ($_SESSION['registerFlag']) {
    ?>
    <div class="message">
        <div class="message__inner">
            <div class="message__text">Вы успешно зарегистрированы.</div>
        </div>
    </div>
    <?php
}
$_SESSION['registerFlag'] = false;
?>
        ...
        if (isset($_POST['submit']) && isset($_POST['login'])) {
            ...
            if (!empty($login) && !empty($password)) {
                ...
                if (!empty($result)) {
                    ...
                    $link->query("UPDATE `users` SET `token`='" . $token . "' WHERE `id` = " . $result['id']);
